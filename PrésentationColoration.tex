...
\item $G$ un graphe avec $\Delta(G)=4$ et $H$ une configuration de $G$.
\item $\varphi:E(G')\to \{1,2,3,4\}$ une 4-arête-coloration de $G'$. 
\item $\varphi' \in N(\varphi)$ une coloration adjacente à $\varphi$ obtenu en alternant les couleurs de la $(c,c')$-chaîne de Kempe de $e\in \partial_G(H)$ pour $c,c'\in\{1,2,3,4\}$.
\item $\beta = \varphi|_{\partial_G(H)}$ et $\beta' = \varphi'|_{\partial_G(H)}$ les colorations de  $\partial_G(H)$ induites par $\varphi$ et $\varphi'$. 
\end{itemize}
Alors,
$$
...
Alors, $\beta$ \emph{s'étend} vers $H$ s'il existe une coloration $\psi$ de $H^*$ telle que $\psi|_{\partial_G(H)} = \beta$. 
...
\begin{frame}
Soit $G$ un graphe planaire. V(G) ensemble de sommets de $G$ et F ensemble de faces.

La \emph{charge} des sommets et des faces
