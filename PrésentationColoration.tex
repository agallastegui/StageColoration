...
\end{theorem}
Si $\chi'(G) = \Delta(G)$,  $G$ est appelé graphe de classe 1.

\pause
Pour un graphe planaire $G$ voici quelques résultats connus
\begin{itemize}
\item $\Delta(G) \geq 8$ $\implies$ $\chi'(G)=8$. (Vizing 1965).
...
\end{proposition}
Propriétés structurelles d'un contre-exemple $G$:
\begin{itemize}
\item Il n'existe pas de sommet $v$ de $G$ tel que $d_G(v) = 1$.
...
\begin{frame}
\begin{itemize}
\item Génération de Couplages de $\mathcal{X}^{c,c'}_\varphi$ $\forall c,c$ et $\forall \varphi$.
...
\begin{itemize}
\item On génère l'ensemble des colorations $\psi$ de $H$ t.q. $\psi|_{\partial_G(H)}=\beta$, c-à-d $\Phi_0$.
\item Pour $i \in \mathbb{N}\cup \{0\}$ on génère l'ensemble $\Phi_{i+1}$ des colorations adjacentes aux colorations de $\cup_{j=0}^i\Phi_i$.
\item L'ensemble totale des colorations qui s'étendent vers $H$ est $\Phi(H) = \bigcup_{i \in \mathbb{N}\cup\{0\}} \Phi_i(H)$.
\item On génère l'ensemble $\Phi'_0$ des colorations $\psi'$ de $H'$ t.q. une coloration de $G'$ s'étend vers $H'$.
\item Regardons si $\Phi'_0(H') \subseteq \Phi(H)$ 
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Lecture de configuration $H$ et sa réduction potentielle présentées sous forme d'une liste d'adjacence.
...
Les arêtes du graphe résiduel sont l'ensemble $M$.
\pause
...
$$
\begin{cases}
k' =  k - (2\beta'(e)+c + c')\cdot 4^{e} \textrm{ si  }e \not\in \mathcal{M} \\
k' = k  - (2\beta'(e)+c + c')\cdot 4^{e} - (2\beta'(e')+c + c')\cdot 4^{e'} \textrm{ si  }ee' \in \mathcal{M}
\end{cases}
$$


Autrement dit,
le premier cas correspond à $d_{\mathcal{X}_{\varphi}^{c,c'}}(e)=0$, 
et le deuxième cas correspond à $d_{\mathcal{X}_{\varphi}^{c,c'}}(e)=1$.
\end{frame}

\begin{frame}
Alors,
pour $k'$ quelconque qui ne s'étend pas,
s'il existe une paire de couleurs $c$, $c'$ pour laquelle n'importe quel couplage des arêtes coloriées avec $c$ ou $c'$ il existe un code $k$ adjacent à $k'$, $k'$ s'étend.
\end{frame}
...
2 avec 1 carré et un pentagone
...
3-3 avec 3 carrés et un pentagone
\end{lemme}
\end{frame}
...
\begin{frame}
\begin{lemme}
Soit $G$ un contre-exemple minimal et $v \in V(G)$ tel que $d_G(v) = 3$ avec que des voisins de grand degré. Alors, il existe une face $f$ incidente à $v$ t.q. $d_G(f) \geq 5$
