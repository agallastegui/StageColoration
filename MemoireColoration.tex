...
$V(\mathcal{X}^{k,l}) = \partial_G(H)$, 
$E(\mathcal{X}^{k,l}) = \{ e_ie_j$, $e_i, e_j \in \partial_G(H)$ tel que $(k,l)$-chaîne de Kempe de $e_i$ est la même que la $(k,l)$-chaîne de Kempe de $e_j\}$ 
\end{definition}
...
\begin{definition}
Soit $H$ une configuration du graphe $G$ non-coloriable et $H'$, une réduction de $H$. Prenons $G'$ le graphe avec $H'$ (à la place de $H$). Alors $\varphi'$ est une coloration quelconque de $G'$.{\color{blue}je bloque ici je verrai plus tard}
\end{definition}

\begin{definition}
Soit $H$ une configuration de $G$. Alors, $\beta' = \varphi'|_{\partial_G(H)}$ est une coloration de $\partial_G(H')=\partial_G(H)$.
\end{definition}

\begin{definition}
Soit $H$ une configuration de $G$ et $\beta$ une coloration de $\partial_G(H)$. Alors, $\beta$ \emph{s'étend} vers $G$ s'il existe une coloration $\psi$ de $H$ telle que $\psi|_{\partial_G(H)} = \beta$. Notons $\Phi_0$ l'ensemble des colorations de $H$ qui s'étendent,
$$
...
\begin{definition}
Soit $C$ une configuration et $\varphi$ une coloration quelconque de $H$. Alors, $\Phi$ est l'ensemble de colorations qui s'étend à $H$. Autrement dit,
$$
\Phi = \{ \varphi, \quad \textrm{telle que $\varphi$ s'étend à $H$} \}.
$$
\label{de:phi}
\end{definition}
